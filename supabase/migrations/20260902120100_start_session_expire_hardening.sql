-- Harden session expiry during start_owned_session.
-- When sessions_lifecycle_status_check omits EXPIRED (pre-20260901150000), expiring
-- a timed-out open row used to abort the whole RPC → start-session 500 SESSION_CREATE_FAILED.

CREATE OR REPLACE FUNCTION public.expire_timed_out_open_session(
  p_session_id uuid,
  p_user_id uuid,
...
REVOKE ALL ON FUNCTION public.expire_timed_out_open_session(uuid, uuid, timestamptz) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.expire_timed_out_open_session(uuid, uuid, timestamptz) FROM anon;
GRANT EXECUTE ON FUNCTION public.expire_timed_out_open_session(uuid, uuid, timestamptz) TO authenticated;
GRANT EXECUTE ON FUNCTION public.expire_timed_out_open_session(uuid, uuid, timestamptz) TO service_role;
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    IF FOUND THEN
      IF v_existing.expires_at IS NOT NULL AND v_existing.expires_at <= v_now THEN
        PERFORM public.expire_timed_out_open_session(v_existing.id, p_user_id, v_now);
      ELSE
...
EXCEPTION
  WHEN unique_violation THEN
    ...
    RETURN jsonb_build_object(
      ...
      'error', 'Could not create session due to a concurrent start.'
    );
END;
$$;
