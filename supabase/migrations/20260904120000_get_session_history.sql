-- Session History: owner-scoped normalized timeline (Approach B).

CREATE INDEX IF NOT EXISTS idx_sessions_user_created_history
  ON public.sessions (user_id, created_at DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_mock_tests_user_updated_history
  ON public.mock_tests (user_id, updated_at DESC NULLS LAST);

CREATE INDEX IF NOT EXISTS idx_practice_workspace_user_started_history
  ON public.practice_workspace_sessions (user_id, started_at DESC);

CREATE INDEX IF NOT EXISTS idx_coding_submissions_user_submitted_history
