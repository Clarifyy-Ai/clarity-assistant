-- Fix: production deduct_credits_service still used request.jwt.claim.role only.
-- PostgREST/supabase-js often populates request.jwt.claims JSON without the
-- singular claim setting, so Edge service-role clients got Forbidden and
-- hybrid ops mapped that to fake INSUFFICIENT_CREDITS (prep-tool, parse-resume).
...
GRANT EXECUTE ON FUNCTION public.is_service_role_request() TO service_role, postgres;
