...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: existing } = await db.from("document_processing_jobs")
      .select("id, status, credits_reserved").eq("id", jobId).eq("owner_id", user.id).maybeSingle();
    ...
    if (existing.status === "cancelled") return json(req, { success: true, idempotent: true, jobId, state: "cancelled", creditsRefunded: false });
