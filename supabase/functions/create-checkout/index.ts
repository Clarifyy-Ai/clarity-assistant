...
// Secure redirect host (your frontend domain)
const PUBLIC_URL = Deno.env.get("PUBLIC_URL") ?? "";
const ALLOWED_REDIRECT_HOST = PUBLIC_URL ? new URL(PUBLIC_URL).hostname : "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!price_id || !success_url || !cancel_url) {
      return new Response(
        JSON.stringify({ error: "Missing price_id, success_url, or cancel_url" }),
    ...
    // -------------------------------
    try {
      const suc = new URL(success_url);
      const can = new URL(cancel_url);

      if (suc.hostname !== ALLOWED_REDIRECT_HOST || can.hostname !== ALLOWED_REDIRECT_HOST) {
