...
export const ProtectedRoute = memo(function ProtectedRoute({
  ...
  if (requireOnboarded || requireOnboarding) {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7572/ingest/ea82b87b-41ef-4cec-a41d-f9c122e76fc2',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'6cff84'},body:JSON.stringify({sessionId:'6cff84',runId:'pre-fix',hypothesisId:'H5',location:'ProtectedRoute.tsx:onboarding',message:'onboarding_gate',data:{route:location.pathname,completed,isOnboarded:Boolean(isOnboarded),isProfileLoaded,outcome:completed?'allow':'redirect_onboarding'},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    if (!completed) {
      logger.info(LogEvents.ROUTE_GUARD_DECISION, {
        route: location.pathname,
