/**
 * Buffer-complete structured JSON parser for AI provider output (Deno).
 * Mirror of src/lib/ai/structuredParse.ts — keep contracts aligned.
 */
...
export function isStarStructuredAnswer(value: unknown): value is StarStructuredAnswer {
  ...
  return `${situation}${task}${action}${result}${fullAnswer}`.trim().length > 0;
}
...
export const REPAIR_JSON_PROMPT =
  "The previous response was not valid JSON. Reply with ONLY the JSON object, no markdown, no commentary.";
