...
test.describe("QA Aug11 legacy + billing redirects", () => {
  test("answer-bank path does not 404", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/(login|app\/answers)(\/|$|\?)/, { timeout: 15_000 });
  });

  test("debriefs path does not 404", async ({ page }) => {
    const res = await page.goto("/app/debriefs");
    expect(res?.status()).not.toBe(404);
    await expect(page).toHaveURL(/\/(login|app\/debriefs)/);
  });
  ...
  test("rooms path redirects to dashboard or login", async ({ page }) => {
    const res = await page.goto("/app/rooms");
    expect(res?.status()).not.toBe(404);
    await expect(page).toHaveURL(/\/(login|app\/dashboard)/);
  });

  test("singular debrief path redirects toward plural debriefs or login", async ({ page }) => {
    const res = await page.goto("/app/debrief");
    expect(res?.status()).not.toBe(404);
    await expect(page).toHaveURL(/\/(login|app\/debriefs)/);
  });
