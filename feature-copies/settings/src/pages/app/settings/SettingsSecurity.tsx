...
export default function SettingsSecurity() {
  ...
  async function startMfaEnroll() {
    ...
    try {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7572/ingest/ea82b87b-41ef-4cec-a41d-f9c122e76fc2',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'6cff84'},body:JSON.stringify({sessionId:'6cff84',runId:'post-fix',hypothesisId:'H6',location:'SettingsSecurity.tsx:startMfaEnroll',message:'mfa_enroll_precheck',data:{factorCount:latest.length,verified:latest.filter((f)=>f.factor_type==='totp'&&f.status==='verified').length,unverified:latest.filter((f)=>f.factor_type==='totp'&&f.status==='unverified').length,statuses:latest.map((f)=>({type:f.factor_type,status:f.status,name:f.friendly_name??null}))},timestamp:Date.now()})}).catch(()=>{});
      // #endregion
