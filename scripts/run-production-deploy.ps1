...
Write-Host "Step 1/5: Frontend env validation..." -ForegroundColor Yellow
npm run validate-env
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`nStep 2/5: Pre-deploy check..." -ForegroundColor Yellow
node scripts/pre-deploy-check.mjs
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`nStep 3/5: Database migrations..." -ForegroundColor Yellow
npx supabase db push
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`nStep 4/5: Edge functions..." -ForegroundColor Yellow
node scripts/deploy-all-edge-functions.mjs
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`nStep 5/5: Smoke test (requires env)..." -ForegroundColor Yellow
