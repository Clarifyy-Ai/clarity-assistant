...
const { data: q } = await client
  ...
  .select("id,title,language,evaluation_mode")
  .limit(3);
console.log("coding questions", q);
...
if (q?.[0]) {
  ...
  console.log("score-coding", r.status, (await r.text()).slice(0, 500));
}

for (const fn of ["submit-test", "issue-course-certificate", "create-test"]) {
  const r = await fetch(`${url}/functions/v1/${fn}`, {
    method: "POST",
    headers,
    body: JSON.stringify({}),
  });
  console.log(fn, r.status, (await r.text()).slice(0, 300));
}
