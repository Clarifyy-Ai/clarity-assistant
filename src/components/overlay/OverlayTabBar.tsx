...
export function OverlayTabBar() {
  ...
  const contextLabel = useMemo(() => {
    const activeResume = resumes?.find?.((r: any) => r?.id === activeResumeId);
    const activeJd = jds?.find?.((j: any) => j?.id === activeJdId);

    if (activeResume?.title) return String(activeResume.title).slice(0, 12);
    if (activeJd?.role_title) return String(activeJd.role_title).slice(0, 12);
    return "Context";
