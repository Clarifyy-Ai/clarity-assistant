...
// ─────────────────────────────────────────────────────────────────
// PrepLab
// STAR Answer Builder + Answer Bank browser.
// ─────────────────────────────────────────────────────────────────

const ANSWER_CATEGORIES: { value: AnswerCategory; label: string; icon: string }[] = [
...
const STAR_PLACEHOLDERS = {
  situation: "Describe the context and background. Where were you? What was happening?",
  task:      "What was your specific responsibility or challenge in this situation?",
  action:    "What steps did YOU take? Be specific and use 'I' not 'we'.",
  result:    "What was the outcome? Quantify where possible (%, time saved, revenue…)",
};

const STAR_COLORS = {
  situation: { label: "S", bg: "bg-blue-500/10",   border: "border-blue-500/30",   text: "text-blue-400"   },
  task:      { label: "T", bg: "bg-yellow-500/10",  border: "border-yellow-500/30", text: "text-yellow-400" },
  action:    { label: "A", bg: "bg-violet-500/10",  border: "border-violet-500/30", text: "text-violet-400" },
  result:    { label: "R", bg: "bg-emerald-500/10", border: "border-emerald-500/30",text: "text-emerald-400" },
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-400 mt-1 text-sm">
            Build compelling STAR answers and manage your personal answer bank
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                ...
                <textarea
                  ...
                  placeholder="e.g. Tell me about a time you led a difficult project under tight deadlines…"
                  rows={3}
                ...
                <div className="flex gap-2 mt-3">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-violet-600/30 hover:bg-violet-600/50 border border-violet-500/40 text-violet-300 text-sm font-medium rounded-xl disabled:opacity-40 transition-all"
                  >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <span className={cn(
                        "w-6 h-6 rounded-md text-xs font-black flex items-center justify-center",
                        style.bg, style.text
                      )}>
                        {style.label}
                      </span>
                      <label className={cn("text-sm font-semibold capitalize", style.text)}>
                        {key}
                      </label>
                    </div>
                    <textarea
                      value={builder.components[key] ?? ""}
                      onChange={(e) => builder.updateComponent(key, e.target.value)}
                      placeholder={STAR_PLACEHOLDERS[key]}
                      rows={key === "action" ? 4 : 3}
                      className="w-full bg-black/20 border border-white/10 text-white placeholder-gray-600 rounded-xl px-3 py-2.5 resize-none focus:outline-none focus:border-white/30 text-sm leading-relaxed"
                    />
                  </div>
                );
              })}

              {/* Action buttons (only when answer exists) */}
              ...
              {builder.analysis && (
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      "w-16 h-16 rounded-full flex items-center justify-center text-xl font-black border-2",
                      builder.analysis.overall_score >= 75
                        ? "border-green-500 text-green-400"
                        : builder.analysis.overall_score >= 50
                        ? "border-yellow-500 text-yellow-400"
                        : "border-red-500 text-red-400"
                    )}>
                      {builder.analysis.overall_score}
                    </div>
                    <div className="flex-1 grid grid-cols-2 gap-2">
                      {(["situation", "task", "action", "result"] as const).map((k) => {
                        const scoreKey = `${k}_score` as keyof typeof builder.analysis;
                        const val = (builder.analysis as any)[scoreKey] as number;
                        const c = STAR_COLORS[k];
                        return (
                          <div key={k} className="flex items-center gap-2">
                            <span className={cn("text-xs font-bold w-3", c.text)}>
                              {c.label}
                            </span>
                            <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                              <div
                                className={cn("h-full rounded-full", c.text.replace("text-", "bg-"))}
                                style={{ width: `${val}%` }}
                              />
                            </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {builder.analysis.has_quantified_result && (
                      <span className="px-2 py-1 bg-green-500/10 border border-green-500/20 text-green-400 text-xs rounded-full flex items-center gap-1">
                    ...
                    <span className={cn(
                      ...
                      builder.analysis.action_specificity === "specific"
                        ? "bg-green-500/10 border-green-500/20 text-green-400"
                        : builder.analysis.action_specificity === "moderate"
                        ? "bg-yellow-500/10 border-yellow-500/20 text-yellow-400"
                        : "bg-red-500/10 border-red-500/20 text-red-400"
                    )}>
                      Action: {builder.analysis.action_specificity}
                    </span>
                  </div>

                  {/* Strengths */}
                  {builder.analysis.strengths.length > 0 && (
                    <div>
                      <p className="text-xs text-green-400 font-medium mb-1.5">Strengths</p>
                      <ul className="space-y-1">
                        {builder.analysis.strengths.map((s, i) => (
                          <li key={i} className="flex items-start gap-2 text-xs text-gray-300">
                            <CheckCircle className="w-3 h-3 text-green-400 mt-0.5 shrink-0" />
                            {s}
                          </li>
                        ))}
                      The file got cut off mid-render. Here is the complete `PrepLab.tsx` from the beginning:

---

## 68. `src/pages/PrepLab.tsx` (complete)
