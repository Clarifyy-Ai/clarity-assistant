 ...
 * Keep in sync with supabase/functions/_shared/sessionShareability.ts.
 */
...
/**
 * True when the session is authoritatively finished with at least one scorable answer.
 * Recovers mislabeled CANCELLED/abandoned rows that still have answers (legacy race).
 */
export function isAuthoritativeSessionComplete(input: {
  ...
  // Historical false-CANCELLED / abandoned-with-answers: treat as complete for scoring/share.
