...
export function generateButtonLabel(decision: InventoryDecision): string {
  if (decision.mode === "hybrid_deterministic") {
    return `Generate Realistic Practice Mock — ${decision.requested} questions`;
  }
  if (decision.mode === "ai_assisted") {
    return `Generate AI Practice Paper — ${decision.requested} questions`;
  }
...
export function generationSourceSummary(decision: InventoryDecision): string {
  ...
  if (decision.mode === "ai_assisted") {
    if (decision.available <= 0) {
      return `All ${decision.requested} questions are AI practice content (not Official/PYQ).`;
    }
    return `${decision.available} from the approved bank, ${decision.aiQuestions} AI practice fill (not Official/PYQ).`;
  }
