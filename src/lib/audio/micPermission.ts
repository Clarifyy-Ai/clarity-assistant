...
export type AcquireMicrophoneResult = {
  stream: MediaStream;
  permission: MicPermissionState;
  /** True only when getUserMedia ran while the Permissions API reported "prompt". */
...
/**
 * Acquire a microphone stream without prompting when permission is already
 * granted or denied. Callers must treat restored session state as independent
...
/** Device-label hydration may call getUserMedia only when it cannot prompt. */
